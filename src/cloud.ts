 ...
 *
 * cloud.ts — Capa de nube (Supabase) para Calzado Multi-Inquilino.
...
export async function validarLicencia(codigo: string): Promise<any | null> {
  try {
    ...
    return (d && typeof d === 'object' && d.codigo) ? d : null;
  } catch (e) { return null; }
}
...
export async function asegurarCuentaSeguraColab(usuario: string, password: string, codigo: string) {
  ...
  if (!sess) {
    let ok = false;
    try { ok = await rpc('calzado_verificar_colab', { p_codigo: codigo, p_usuario: usuario, p_pass: password }, false); }
    catch (e) { ok = false; }
  ...
  try { await rpc('calzado_unir_colab', { p_codigo: codigo, p_usuario: usuario }); }
...
// ── Sync de datos del local (tabla calzado_backups, RLS por membresía) ──
export async function cloudLoad(codigo: string): Promise<CloudData | null> {
  const bearer = (await authToken()) || SB_KEY;
  try {
    const res = await fetch(
      `${SB_URL}/rest/v1/calzado_backups?tenant_id=eq.${encodeURIComponent(codigo)}&select=datos&limit=1`,
...
export async function cloudSave(codigo: string, datos: CloudData): Promise<boolean> {
  const bearer = (await authToken()) || SB_KEY;
  try {
    const res = await fetch(`${SB_URL}/rest/v1/calzado_backups`, {
      method: 'POST',
      headers: {
...
// ── Público (catálogo) y alta de pedido ───────────────────────────────
export async function calzadoPublica(codigo: string): Promise<CloudData | null> {
  try { return await rpc('calzado_publica', { p_codigo: codigo }, false) as CloudData; }
  catch (e) { return null; }
}

export async function calzadoAgregarPedido(codigo: string, pedido: any): Promise<void> {
